...
import { APPLICANT_STATUS_MAP, APPLICANT_STATUS_MESSAGE_MAP, assertValidStatusTransition } from "../../lib/applicant-status.js";
 ...
 */
applicationRouter.post("/", authenticate, authorize("job_seeker"), validate({ body: applyJobSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { jobId, isAnonymous, cvUrl } = req.body;
        ...
        const [existing] = await db
            ...
            .where(and(eq(schema.jobApplications.jobId, jobId), eq(schema.jobApplications.userId, userId)));
        if (existing)
 ...
 */
applicationRouter.get("/", authenticate, authorize("job_seeker"), validate({ query: applicationQuerySchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        ...
        const [rows, [{ total }]] = await Promise.all([
            db
                ...
                .from(schema.jobApplications)
                .innerJoin(schema.jobListings, eq(schema.jobApplications.jobId, schema.jobListings.jobId))
        ...
        return res.json(successResponse("Applications fetched successfully", {
            applications,
            pagination: {
                page,
                limit,
 ...
 */
applicationRouter.get("/:id/status", authenticate, authorize("job_seeker"), validate({ params: applicationParamsSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const [application] = await db
            ...
            .innerJoin(schema.jobListings, eq(schema.jobApplications.jobId, schema.jobListings.jobId))
            .innerJoin(schema.companies, eq(schema.jobListings.companyId, schema.companies.companyId))
            .where(and(eq(schema.jobApplications.applicationId, id), eq(schema.jobApplications.userId, userId)));
        if (!application)
            throw new HttpError(404, "Application not found");
        return res.json(successResponse("Application status fetched", {
            ...application,
 ...
 */
jobApplicantsRouter.get("/:jobId/applicants", authenticate, authorize("recruiter"), validate({ params: jobApplicantsParamsSchema, query: applicantListQuerySchema }), async (req, res, next) => {
    try {
        ...
        const [rows, [{ total }]] = await Promise.all([
            db
                ...
                .from(schema.jobApplications)
                .leftJoin(schema.jobSeekerProfiles, eq(schema.jobApplications.userId, schema.jobSeekerProfiles.userId))
        ...
        const totalPages = Math.ceil(Number(total) / limit);
        return res.json(successResponse("Applicants fetched successfully", {
            applicants,
            pagination: {
                page,
                limit,
 ...
 *         description: Application not found
 */
applicationRouter.get("/:id", authenticate, authorize("recruiter"), validate({ params: applicationParamsSchema }), async (req, res, next) => {
    try {
        const { id } = req.params;
        const [row] = await db
            .select({
 ...
 *         description: Application not found
 */
applicationRouter.patch("/:id/status", authenticate, authorize("recruiter"), validate({ params: applicationParamsSchema, body: updateStatusSchema }), async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        assertValidStatusTransition(application.recruiterStatus, newStatus);
        const [updated] = await db
            .update(schema.jobApplications)
            .set({ recruiterStatus: newStatus, updatedAt: new Date() })
            .where(eq(schema.jobApplications.applicationId, id))
            .returning();
        ...
        await createNotification({
            ...
            body: `Your application for "${job?.title ?? "a job"}" has moved to: ${APPLICANT_STATUS_MAP[newStatus]}.`,
            referenceId: id
        });
        return res.json(successResponse("Application status updated", {
