...
notificationRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const { page, limit, isRead, type } =
        req.query as unknown as z.infer<typeof notificationQuerySchema>;
...
notificationRouter.patch(
  "/:id/read",
  authenticate,
  validate({ params: notifParamsSchema }),
  async (req, res, next) => {
    try {
      ...
      const [notif] = await db
        ...
        .where(
          and(
            eq(schema.notifications.notificationId, id),
            eq(schema.notifications.userId, req.user!.userId)
          )
        );
