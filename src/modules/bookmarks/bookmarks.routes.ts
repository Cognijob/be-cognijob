...
bookmarkRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (existing) throw new HttpError(409, "Job already bookmarked");
      ...
      return res.status(201).json(
        successResponse("Job bookmarked successfully", {
    ...
    try {
      ...
      const [bookmarks, [{ total }]] = await Promise.all([
        db
          ...
          .innerJoin(
            schema.jobListings,
            eq(schema.bookmarks.jobId, schema.jobListings.jobId)
          )
          .innerJoin(
            schema.companies,
            eq(schema.jobListings.companyId, schema.companies.companyId)
      ...
      return res.json(
        successResponse("Bookmarks fetched successfully", {
          bookmarks,
          total: Number(total),
        })
      );
...
bookmarkRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      return res.json(
        successResponse("Bookmark status checked", {
