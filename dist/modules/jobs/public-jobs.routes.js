 ...
 */
publicJobRouter.get("/", validate({ query: publicJobQuerySchema }), async (req, res, next) => {
    try {
        const { page, limit, search, location, category, employment_type, level, skills, sort, order } = req.query;
        const offset = (page - 1) * limit;
        const filters = [
            eq(schema.jobListings.status, "published"),
            // Exclude expired jobs
            or(isNull(schema.jobListings.expiresAt), gt(schema.jobListings.expiresAt, new Date())),
            search
                ? or(ilike(schema.jobListings.title, `%${search}%`), ilike(schema.jobListings.description, `%${search}%`), ilike(schema.jobListings.category, `%${search}%`), ilike(schema.jobListings.skills, `%${search}%`))
                : undefined,
        ...
        const sortCol = sort === "expires_at" ? schema.jobListings.expiresAt : schema.jobListings.createdAt;
        ...
        return res.json(successResponse("Jobs fetched successfully", {
            jobs,
            pagination: {
                page,
                limit,
 ...
 */
publicJobRouter.get("/:id", validate({ params: publicJobParamsSchema }), async (req, res, next) => {
    try {
        const { id } = req.params;
        const [job] = await db
            .select(publicJobSelect)
            .from(schema.jobListings)
            .innerJoin(schema.companies, eq(schema.companies.companyId, schema.jobListings.companyId))
            .where(and(eq(schema.jobListings.jobId, id), eq(schema.jobListings.status, "published")));
        if (!job)
