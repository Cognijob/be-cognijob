...
userRouter.get("/profile", authenticate, async (req, res, next) => {
    try {
        ...
        return res.json(successResponse("Profile fetched successfully", {
            ...user,
...
userRouter.get("/profile/preview", authenticate, authorize("job_seeker"), async (req, res, next) => {
    try {
        ...
        const [profile] = await db
            ...
            .from(schema.jobSeekerProfiles)
            .where(eq(schema.jobSeekerProfiles.userId, userId));
        return res.json(successResponse("Profile preview fetched successfully", {
            userId: user.userId,
            name: user.name,
 ...
 */
userRouter.put("/profile", authenticate, validate({ body: updateProfileSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const body = req.body;
        ...
        if (body.age !== undefined)
            userFields.age = body.age;
        if (body.location !== undefined)
            userFields.location = body.location;
        if (body.whatsappNumber !== undefined)
            userFields.whatsappNumber = body.whatsappNumber;
        ...
        if (body.skills !== undefined)
            profileFields.skills = body.skills;
        if (body.portfolioLink !== undefined)
            profileFields.portfolioLink = body.portfolioLink;
        if (body.workExperience !== undefined)
            profileFields.workExperience = body.workExperience;
        if (body.awards !== undefined)
            profileFields.awards = body.awards;
        if (body.organizationExperience !== undefined)
            profileFields.organizationExperience = body.organizationExperience;
        if (body.interests !== undefined)
            profileFields.interests = body.interests;
 ...
 */
userRouter.post("/cv", authenticate, authorize("job_seeker"), upload.single("cv"), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const file = req.file;
        if (!file)
        ...
        return res.json(successResponse("CV uploaded successfully", {
            ...cvFields,
            profileCompleteness,
        }));
    }
 ...
 */
userRouter.post("/photo", authenticate, upload.single("photo"), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const file = req.file;
        if (!file)
 ...
 */
userRouter.post("/change-password", authenticate, validate({ body: changePasswordSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
