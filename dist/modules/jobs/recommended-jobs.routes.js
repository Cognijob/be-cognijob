 ...
 */
recommendedJobRouter.get("/", authenticate, authorize("job_seeker"), validate({ query: recommendedQuerySchema }), async (req, res, next) => {
    try {
        ...
        const baseFilters = [
            ...
            or(sql `${schema.jobListings.expiresAt} IS NULL`, gt(schema.jobListings.expiresAt, new Date())),
        ...
        if (skillTokens.length > 0) {
            ...
            const skillMatchFilter = or(...skillTokens.flatMap((skill) => {
                const like = `%${skill}%`;
                return [
            ...
            const scoreTerms = skillTokens.map((skill) => sql `(
            ...
            const relevanceExpr = scoreTerms.reduce((acc, curr) => sql `${acc} + ${curr}`, sql `0`);
            const rows = await db
                .select({
        ...
        return res.json(successResponse("Recommended jobs fetched successfully", {
            matchedBySkills,
            jobs,
        }));
    }
