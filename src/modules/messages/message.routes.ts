...
messageRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const { applicationId } = req.body as z.infer<typeof createConversationSchema>;
      ...
      const [isMember] = await db
        ...
        .where(
          and(
            eq(schema.companyRecruiters.companyId, job.companyId),
...
messageRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const { page, limit } = req.query as unknown as z.infer<typeof conversationQuerySchema>;
      ...
      const withUnread = convs.map((c) => ({
        ...c,
        unreadCount:
          role === "job_seeker" ? c.unreadBySeeker : c.unreadByRecruiter
      }));
...
messageRouter.get(
  "/conversations/:id/messages",
  authenticate,
  validate({ params: convParamsSchema, query: messageQuerySchema }),
  async (req, res, next) => {
    try {
      const userId = req.user!.userId;
      const { id } = req.params as { id: string };
      const { limit, before } = req.query as unknown as z.infer<typeof messageQuerySchema>;
...
messageRouter.post(
  "/conversations/:id/messages",
  authenticate,
  validate({ params: convParamsSchema, body: sendMessageSchema }),
  async (req, res, next) => {
    try {
      const userId = req.user!.userId;
      const { id } = req.params as { id: string };
      const { body: msgBody } = req.body as z.infer<typeof sendMessageSchema>;
...
messageRouter.delete(
  "/conversations/:convId/messages/:msgId",
  authenticate,
  validate({ params: msgParamsSchema }),
  async (req, res, next) => {
    try {
      const userId = req.user!.userId;
      ...
      const [msg] = await db
        ...
        .where(
          and(
            eq(schema.messages.messageId, msgId),
      ...
      if (msg.deletedAt) throw new HttpError(410, "Message already deleted");
