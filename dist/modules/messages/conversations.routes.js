...
async function ensureConversationAccess(conversationId, userId) {
    ...
    const isParticipant = conv.jobSeekerId === userId || conv.recruiterId === userId;
    if (!isParticipant) {
 ...
 */
conversationRouter.post("/applications/:applicationId", authenticate, validate({ params: applicationIdParamsSchema }), async (req, res, next) => {
    try {
        ...
        return res.status(201).json(successResponse("Conversation opened successfully", conversation));
    }
    catch (error) {
        return next(error);
    }
...
conversationRouter.get("/", authenticate, async (req, res, next) => {
    try {
        ...
        const conversations = await db
            ...
            .innerJoin(schema.jobApplications, eq(schema.conversations.applicationId, schema.jobApplications.applicationId))
            .leftJoin(schema.users, eq(schema.conversations.jobSeekerId, schema.users.userId))
            .where(whereClause)
 ...
 */
conversationRouter.get("/:id", authenticate, validate({ params: conversationIdParamsSchema, query: messagesQuerySchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        ...
        const [messages, [{ total }]] = await Promise.all([
            db
                ...
                .where(and(eq(schema.messages.conversationId, id), 
            // Exclude soft-deleted messages
            eq(schema.messages.deletedAt, null)))
            ...
            db
                .select({ total: count() })
                .from(schema.messages)
                .where(and(eq(schema.messages.conversationId, id), eq(schema.messages.deletedAt, null))),
        ]);
        const totalPages = Math.ceil(Number(total) / limit);
        return res.json(successResponse("Conversation fetched successfully", {
            conversation: conv,
            messages,
            pagination: {
                page, limit,
 ...
 *         description: Conversation not found
 */
conversationRouter.post("/:id/messages", authenticate, validate({ params: conversationIdParamsSchema, body: sendMessageSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
 ...
 *         description: Conversation not found
 */
conversationRouter.patch("/:id/read", authenticate, validate({ params: conversationIdParamsSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const conv = await ensureConversationAccess(id, userId);
