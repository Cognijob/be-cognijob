 ...
 */
ratingRouter.post("/", authenticate, authorize("job_seeker"), validate({ body: createRatingSchema }), async (req, res, next) => {
    try {
        const userId = req.user.userId;
        const { companyId, ratingScore, review } = req.body;
        // Cek company ada
        ...
        const [hasApplied] = await db
            ...
            .where(and(eq(schema.jobApplications.userId, userId), eq(schema.jobListings.companyId, companyId)))
            .limit(1);
        if (!hasApplied) {
        ...
        const [existingRating] = await db
            ...
            .where(and(eq(schema.workplaceRatings.companyId, companyId), eq(schema.workplaceRatings.userId, userId)));
        if (existingRating) {
 ...
 */
ratingRouter.get("/companies/:id/ratings", validate({ params: companyParamsSchema, query: ratingQuerySchema }), async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        return res.json(successResponse("Ratings fetched successfully", {
            companyId: id,
