...
companyRouter.get(
  "/companies",
  validate({ query: publicCompanyQuerySchema }),
  async (req, res, next) => {
    try {
      const { search, industry, location, size, page, limit } =
        req.query as unknown as z.infer<typeof publicCompanyQuerySchema>;
      ...
      return res.json(
        successResponse("Companies fetched successfully", {
          companies,
          pagination: {
            page, limit,
            total: Number(total),
  ...
  async (req, res, next) => {
    try {
      ...
      const [[{ followerCount }], ratingRows] = await Promise.all([
        ...
        db
          .select({ avgRating: sql<number>`AVG(${schema.workplaceRatings.ratingScore})` })
      ...
      return res.json(
        successResponse("Company detail fetched successfully", {
          ...company,
  ...
  async (req, res, next) => {
    try {
      ...
      const [existing] = await db
        ...
        .where(
          and(
            eq(schema.companyFollows.companyId, companyId),
            eq(schema.companyFollows.userId, userId)
          )
        );
 
      if (existing) throw new HttpError(409, "You are already following this company");
      ...
      return res.status(201).json(
        successResponse("Company followed successfully", {
...
companyRouter.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!existing) throw new HttpError(404, "You are not following this company");
      ...
      return res.json(
        successResponse("Company unfollowed successfully", {
          companyId,
          followerCount: Number(followerCount),
        })
      );
...
companyRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      return res.json(
        successResponse("Follow status fetched", {
  ...
  async (req, res, next) => {
    try {
      ...
      const rows = await db
        ...
        .innerJoin(
          schema.companies,
          eq(schema.companyFollows.companyId, schema.companies.companyId)
        )
        .where(eq(schema.companyFollows.userId, userId))
        .orderBy(desc(schema.companyFollows.followedAt));
 
      return res.json(
        successResponse("Followed companies fetched successfully", { companies: rows })
      );
    } catch (error) {
      return next(error);
    }
