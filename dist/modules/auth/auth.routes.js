...
import { forgotPasswordSchema, jobSeekerRegistrationSchema, loginSchema, recruiterRegistrationSchema, resetPasswordSchema } from "./auth.schemas.js";
 ...
 */
authRouter.post("/register/job-seeker", validate({ body: jobSeekerRegistrationSchema }), async (req, res, next) => {
    try {
 ...
 */
authRouter.post("/register/recruiter", validate({ body: recruiterRegistrationSchema }), async (req, res, next) => {
    try {
        const payload = req.body;
        const [existingUser] = await db
            .select({ userId: schema.users.userId })
...
authRouter.post("/forgot-password", validate({ body: forgotPasswordSchema }), async (req, res, next) => {
    try {
        ...
        if (!user) {
            return res.json(successResponse("Jika email Anda terdaftar, instruksi pemulihan kata sandi telah dikirim ke email Anda."));
        }
        ...
        return res.json(successResponse("Tautan atur ulang kata sandi telah dikirim ke email Anda", data));
    }
    catch (error) {
        return next(error);
    }
