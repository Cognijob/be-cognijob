import { boolean, date, check, index, integer, pgEnum, pgTable, text, timestamp, unique, uuid, varchar, jsonb } from "drizzle-orm/pg-core";
...
// ─── COMPANY RECRUITERS ───────────────────────────────────────────────────────
export const companyRecruiters = pgTable("company_recruiters", {
...
export const jobSeekerProfiles = pgTable("job_seeker_profiles", {
    ...
    completenessCheck: check("job_seeker_profiles_completeness_check", sql `${table.profileCompleteness} BETWEEN 0 AND 100`)
}));
// ─── JOB APPLICATIONS ────────────────────────────────────────────────────────
...
// ─── WORKPLACE RATINGS ────────────────────────────────────────────────────────
export const workplaceRatings = pgTable("workplace_ratings", {
    ...
    scoreCheck: check("workplace_ratings_score_check", sql `${table.ratingScore} BETWEEN 1 AND 5`),
