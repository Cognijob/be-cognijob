...
const ensurePublishableJobPayload = (payload: {
  ...
  if (missingFields.length > 0) {
    throw new HttpError(
      400,
      `Published jobs require complete data: ${missingFields.join(", ")}`
    );
  }
};
  ...
  async (req, res, next) => {
    try {
      ...
      return res.status(201).json(
        successResponse(
          payload.status === "draft" ? "Job draft saved successfully" : "Job created successfully",
          job
        )
      );
    } catch (error) {
...
jobRouter.put(
  ...
  authenticate,
  authorize("recruiter"),
  validate({ params: jobParamsSchema, body: updateJobSchema }),
  async (req, res, next) => {
    try {
      const { id } = req.params as { id: string };
      const payload = req.body as typeof req.body;
      ...
      return res.json(
        successResponse(
          nextStatus === "draft" ? "Job draft saved successfully" : "Job updated successfully",
          job
        )
      );
    } catch (error) {
